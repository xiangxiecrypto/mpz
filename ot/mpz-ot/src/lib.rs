...
pub trait RevealMessages: ProtocolMessage {
    ...
    ///
    /// * `channel` - The channel to communicate with the receiver.
    async fn reveal<C: Duplex<Self::Msg>>(&mut self, channel: &mut C) -> Result<(), OTError>;
}
...
pub trait VerifyChoices<T>: ProtocolMessage {
    ...
    ///
    /// * `channel` - The channel to communicate with the receiver.
    async fn verify_choices<C: Duplex<Self::Msg>>(&mut self, channel: &mut C)
        -> Result<T, OTError>;
}
...
pub trait OTReceiver<T, U>: ProtocolMessage {
    ...
    /// # Arguments
    ///
    /// * `channel` - The channel to communicate with the sender.
...
pub trait VerifyMessages<T>: ProtocolMessage {
    ...
    async fn verify<C: Duplex<Self::Msg>>(
        &mut self,
        channel: &mut C,
        msgs: &[T],
    ) -> Result<(), OTError>;
