...
impl Receiver {
    ...
    pub fn setup() -> Receiver<state::Extension> {
        Receiver {
...
impl Receiver<state::Extension> {
    ...
    pub fn extend(&mut self, ms: &[Block], us: &[bool], d: usize) -> Result<(), ReceiverError> {
        if ms.len() != us.len() {
        ...
        for i in 0..(2 * d - 1) {
            ...
            mi[i] = Block::inn_prdt_red(&m, &base);

            ui[i] =
                u.into_iter().zip(base.iter()).fold(
                    Block::ZERO,
                    |acc, (b, base)| {
                        if b {
                            acc ^ *base
        ...
        for i in 0..d - 1 {
            poly_update(&mut gi, mi[i], ui[i], i + 2);
            gi[0] ^= mi[d + i];
            gi[1] ^= ui[d + i];
        }

        self.state.ais.push(gi);
...
pub mod state {
    ...
    pub struct Extension {
        /// receiver's output blocks, support multiple extension.
        pub(super) ais: Vec<Vec<Block>>,
